...
namespace Hirudo\Libs\Impl\StandAlone\lib;
/**
...
class JURI {
    ...
    static function &getInstance($uri = 'SERVER') {
        ...
        if (!isset(self::$instances[$uri])) {
            ...
            if ($uri == 'SERVER') {
                ...
                /*
                 * Since we are assigning the URI from the server privateiables, we first need
                ...
                if (!empty($_SERVER['PHP_SELF']) && !empty($_SERVER['REQUEST_URI'])) {
                     ...
                     * QUERY_STRING environment privateiables.
                     */
                } else {
                    // IIS uses the SCRIPT_NAME privateiable instead of a REQUEST_URI privateiable... thanks, MS
                ...
                // Now we need to clean what we got since we can't trust the server private
    ...
    function base($pathonly = false) {
        ...
        if (!isset(self::$base)) {
            ...
            if (strpos(php_sapi_name(), 'cgi') !== false && !empty($_SERVER['REQUEST_URI'])) {
                //Apache CGI
                self::$base['path'] = rtrim(dirname($_SERVER['PHP_SELF']), '/\\');
            } else {
                //Others
                self::$base['path'] = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\');
            }
        }
    ...
    function parse($uri) {
        ...
        if (isset($_parts['query']))
            parse_str($_parts['query'], $this->_privates);
        return $retval;
    }
    ...
    /**
     * Adds a query privateiable and value, replacing the value if it
     * already exists and returning the old value.
     *
     * @access	public
     * @param	string $name Name of the query privateiable to set
     * @param	string $value Value of the query privateiable
     * @return	string Previous value for the query privateiable
     * @since	1.5
     */
    function setprivate($name, $value) {
        $tmp = @$this->_privates[$name];
        $this->_privates[$name] = $value;
    ...
    /**
     * Returns a query privateiable by name
     *
     * @access	public
     * @param	string $name Name of the query privateiable to get
     * @return	array Query privateiables
     * @since	1.5
     */
    function getprivate($name = null, $default = null) {
        if (isset($this->_privates[$name])) {
            return $this->_privates[$name];
        }
        return $default;
    }

    /**
     * Removes an item from the query string privateiables if it exists
     *
     * @access	public
     * @param	string $name Name of privateiable to remove
     * @since	1.5
     */
    function delprivate($name) {
        if (in_array($name, array_keys($this->_privates))) {
            unset($this->_privates[$name]);
    ...
    function setQuery($query) {
        if (!is_array($query)) {
            ...
            parse_str($query, $this->_privates);
        }

        if (is_array($query)) {
            $this->_privates = $query;
        }
    ...
    function getQuery($toArray = false) {
        ...
        if (is_null($this->_query)) {
            $this->_query = $this->buildQuery($this->_privates);
        }
